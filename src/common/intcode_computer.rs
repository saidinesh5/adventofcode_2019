...
impl IntCodeComputer {
    pub fn new(text: &str) -> IntCodeComputer {
        IntCodeComputer {
            is_halted: false,
            p: 0,
            relative_base: 0,
...
mod tests {
    ...
    fn fetch_output_for(text: &str, input: &mut Vec<isize>) -> Vec<isize> {
    ...
    fn test_position_and_immediate_mode() {
        ...
        assert_eq!(fetch_output_for("101,0,0,0,4,0,99", &mut vec![1]), [101]);

        // 1001,0,0,0,99 becomes 1001,0,0,0,99 (1001 + 0 = 101).
        assert_eq!(fetch_output_for("1001,0,0,0,4,0,99", &mut vec![1]), [1001]);

        // 1101,1,238,0,99 becomes 239,1,238,0,99 (1 + 238 = 101).
        assert_eq!(fetch_output_for("1101,1,238,0,4,0,99", &mut vec![1]), [239]);

        // 1,0,0,0,99 becomes 2,0,0,0,99 (1 + 1 = 2).
        assert_eq!(fetch_output_for("1,0,0,0,4,0,99", &mut vec![1]), [2]);

        // The program 3,0,4,0,99 outputs whatever it gets as input, then halts.
        assert_eq!(fetch_output_for("3,0,4,0,99", &mut vec![5]), [5]);
        ...
        assert_eq!(fetch_output_for("1002,4,3,4,33", &mut vec![5]), []);

        // Integers can be negative: 1101,101,-1,0 is a valid program (find 101 + -1, store the result in position 0)
        assert_eq!(fetch_output_for("1101,101,-1,0,4,0,99,0", &mut vec![5]), [100]);

        // Using position mode, consider whether the input is equal to 8; output 1 (if it is) or 0 (if it is not)
        assert_eq!(fetch_output_for("3,9,8,9,10,9,4,9,99,-1,8", &mut vec![5]), [0]);
        assert_eq!(fetch_output_for("3,9,8,9,10,9,4,9,99,-1,8", &mut vec![8]), [1]);

        // 3,9,7,9,10,9,4,9,99,-1,8 - Using position mode, consider whether the input is less than 8; output 1 (if it is) or 0 (if it is not).
        assert_eq!(fetch_output_for("3,9,7,9,10,9,4,9,99,-1,8", &mut vec![8]), [0]);
        assert_eq!(fetch_output_for("3,9,7,9,10,9,4,9,99,-1,8", &mut vec![5]), [1]);

        // Using immediate mode, consider whether the input is equal to 8; output 1 (if it is) or 0 (if it is not).
        assert_eq!(fetch_output_for("3,3,1108,-1,8,3,4,3,99", &mut vec![5]), [0]);
        assert_eq!(fetch_output_for("3,3,1108,-1,8,3,4,3,99", &mut vec![8]), [1]);

        // Using immediate mode, consider whether the input is less than 8; output 1 (if it is) or 0 (if it is not)
        assert_eq!(fetch_output_for("3,3,1107,-1,8,3,4,3,99", &mut vec![8]), [0]);
        assert_eq!(fetch_output_for("3,3,1107,-1,8,3,4,3,99", &mut vec![5]), [1]);
    }

    #[test]
    fn test_relative_mode() {
        // Set the relative base to 1 and output the relative position -1
        assert_eq!(fetch_output_for("109,1,204,-1, 99", &mut vec![]), [109]);

        // 109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99 takes no input and produces a copy of itself as output.
        assert_eq!(fetch_output_for("109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99", &mut vec![]),
                   [109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99]);

        // 1102,34915192,34915192,7,4,7,99,0 should output a 16-digit number
        assert_eq!(fetch_output_for("1102,34915192,34915192,7,4,7,99,0", &mut vec![])[0].to_string().bytes().len(), 16);

        // 104,1125899906842624,99 should output the large number in the middle.
        assert_eq!(fetch_output_for("104,1125899906842624,99", &mut vec![]), [1125899906842624]);
    }
    ...
    fn test_jumps() {
        ...
        // (using position mode)
        assert_eq!(fetch_output_for("3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9", &mut vec![0]), [0]);
        assert_eq!(fetch_output_for("3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9", &mut vec![5]), [1]);

        // (using immediate mode)
        assert_eq!(fetch_output_for("3,3,1105,-1,9,1101,0,0,12,4,12,99,1", &mut vec![0]), [0]);
        assert_eq!(fetch_output_for("3,3,1105,-1,9,1101,0,0,12,4,12,99,1", &mut vec![5]), [1]);
        ...
        assert_eq!(fetch_output_for(program, &mut vec![7]), [999]);
        assert_eq!(fetch_output_for(program, &mut vec![8]), [1000]);
        assert_eq!(fetch_output_for(program, &mut vec![9]), [1001]);
    }
}